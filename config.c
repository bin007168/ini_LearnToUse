...
typedef struct _tagConfigEntry {
	...
	int valueLen;
	//!
	char comment[MAX_LEN_ENTRY_COMMENT];
	int commLen;
	List  listEntry;
...
int openConfigFile(const char * fn)
{
	if (NULL == fn)
	{
		return -1;
...
int addConfigGroup(char * grpName)
{
	...
	if (grp!=NULL) {
		...
		memset(grp->listConfigEntry.comment, 0, MAX_LEN_ENTRY_VALUE);
		grp->listConfigEntry.valueLen=0;
		grp->listConfigEntry.nameLen=0;
		grp->listConfigEntry.commLen=0;
		//INIT_LIST_HEAD(&configGroup.listGroup);
		...
		if (ptrValue!=NULL) // add a new entry
		{
			...
			if ((ptrValue-ptr)<MAX_LEN_ENTRY_NAME) {
					...
					(*ptrValue == '\''))
				{
					...
					++ptrValue;  // Ignore the start delimitation char
					do
					{
				...
				//!
				char *ptrComm = ptrValue;
				while(*ptrComm == '\t' || *ptrComm == ' ')
					ptrComm++;
				int commlen=0;
				if(*ptrComm == ';' || *ptrComm == '#')
				{
					...
					while((*ptrComm != '\0')  &&
						(*ptrComm != '\n')  &&
						(*ptrComm != '\r')  &&
						(*ptrComm != '\t')  && len<MAX_LEN_ENTRY_VALUE)
					{
						commBuf[commlen]=*ptrComm;
						++commlen;
						++ptrComm;
					}
				}
				...
				if (1) {
					...
					if (entry!=NULL) {
						...
						memset(entry->name, 0, MAX_LEN_ENTRY_NAME);
						memset(entry->value, 0, MAX_LEN_ENTRY_VALUE);
						memset(entry->comment,0,MAX_LEN_ENTRY_COMMENT);
						entry->commLen=commlen;
						entry->valueLen=len;
						entry->nameLen=name_len;
						memcpy(entry->comment,ptrComm-commlen,commlen);
		...
		list_for_each_safe(pos_grp, n_grp, &configGroup.listGroup) {
			...
			if (grp && !list_empty_careful(&grp->listConfigEntry.listEntry)) {
				list_for_each_safe(pos_entry, n_entry, &grp->listConfigEntry.listEntry) {
					...
					if (entry) {
						if(entry->commLen>0)
							fprintf(fp, "%s=%s\t;%s\n", entry->name, entry->value,entry->comment);
						else
							fprintf(fp, "%s=%s\n", entry->name, entry->value);
						fflush(fp);
					}
...
int initConfigs()
{
	...
	memset(configGroup.listConfigEntry.name, 0, MAX_LEN_ENTRY_NAME);
	memset(configGroup.listConfigEntry.value, 0, MAX_LEN_ENTRY_VALUE);
	memset(configGroup.listConfigEntry.comment, 0, MAX_LEN_ENTRY_VALUE);
	configGroup.listConfigEntry.valueLen=0;
	configGroup.listConfigEntry.nameLen=0;
	configGroup.listConfigEntry.commLen=0;
	curConfigGroup=NULL;
	curConfigEntry=NULL;
